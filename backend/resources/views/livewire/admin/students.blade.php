<div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-left">
                        ...
                        <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Phone</th>
                        <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Email</th>
                        <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Courses</th>
                    ...
                    @forelse ($students as $student)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-5 py-3.5 font-medium text-gray-800">
                                <button wire:click="openView({{ $student->id }})" class="hover:text-indigo-600 text-left transition-colors">
                                    {{ $student->name }}
                                </button>
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-right whitespace-nowrap">
                                <button
                                    wire:click="openView({{ $student->id }})"
                                ...
                                <button
                                    wire:click="openEdit({{ $student->id }})"
        ...
        >
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <form wire:submit="save" class="px-6 py-5 space-y-4" novalidate>
                    ...
                    {{-- Name --}}
                    <div>
                        <label for="student-name" class="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    {{-- Email --}}
                    <div>
                        <label for="student-email" class="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500
                                   {{ $errors->has('date_of_birth') ? 'border-red-400' : 'border-gray-300' }}"
                        >
                    ...
                    <div x-data="{ open: false }" @keydown.escape.window="open = false">
                        <label for="student-courses" class="block text-sm font-medium text-gray-700 mb-1">
                        ...
                        <div class="relative">
                            <button
                                ...
                                class="w-full flex items-center justify-between gap-3 border rounded-lg px-3 py-2 text-sm text-left focus:outline-none focus:ring-2 focus:ring-indigo-500
                                       {{ $errors->has('courseIds') || $errors->has('courseIds.*') ? 'border-red-400' : 'border-gray-300' }}"
                                :aria-expanded="open"
                            >
                                <span class="truncate {{ count($courseIds) ? 'text-gray-800' : 'text-gray-400' }}">
                                    {{ count($courseIds) ? $courses->whereIn('id', $courseIds)->pluck('name')->join(', ') : 'Choose course' }}
                                </span>
                            ...
                            <div
                                x-show="open"
                                x-transition
                                class="absolute z-20 mt-1 w-full max-h-52 overflow-y-auto rounded-lg border border-gray-200 bg-white p-1 shadow-lg"
                            >
                                @foreach ($courses as $course)
                                    <label class="flex items-center gap-3 rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-indigo-50 cursor-pointer">
                                        <input
                                            type="checkbox"
                                            wire:model.live="courseIds"
                                            value="{{ $course->id }}"
                        ...
                        @error('courseIds') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        @error('courseIds.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    {{-- Status --}}
                    <div>
                        <label for="student-status" class="block text-sm font-medium text-gray-700 mb-1">
    ...
    @if ($showView && $viewStudent)
        ...
        >
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h2 id="view-modal-title" class="text-base font-semibold text-gray-800">Student Details</h2>
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-0.5
                                {{ $viewStudent->status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ ucfirst($viewStudent->status) }}
                            </span>
                        </div>
                    </div>
                    ...
                    <dl class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                        <div>
                            <dt class="text-gray-500 text-xs font-medium uppercase tracking-wide mb-0.5">Phone</dt>
                        ...
                        <div>
                            ...
                            <dd class="text-gray-800">{{ $viewStudent->created_at->format('d M Y') }}</dd>
                        </div>
                        @if ($viewStudent->address)
                            <div class="col-span-2">
                                <dt class="text-gray-500 text-xs font-medium uppercase tracking-wide mb-0.5">Address</dt>
                                <dd class="text-gray-800">{{ $viewStudent->address }}</dd>
                            </div>
                        @endif
                    </dl>
                    ...
                    <div>
                        <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                            Enrolled Courses ({{ $viewStudent->courses->count() }})
                        </h3>
                        @if ($viewStudent->courses->isEmpty())
                            <p class="text-sm text-gray-400">Not enrolled in any courses yet.</p>
                        @else
                            <ul class="divide-y divide-gray-100 rounded-lg border border-gray-200 overflow-hidden">
                                @foreach ($viewStudent->courses as $course)
                                    <li class="px-4 py-2.5 flex items-center justify-between gap-3 text-sm">
                                        ...
                                        <div class="flex items-center gap-2 flex-shrink-0">
                                            <span class="text-xs text-gray-400">{{ $course->pivot->enrolled_at?->format('d M Y') }}</span>
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                        <button
                            wire:click="openEdit({{ $viewStudent->id }})"
        ...
        >
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h2 id="delete-modal-title" class="text-base font-semibold text-gray-800 mb-1">Delete Student</h2>
                        <p class="text-sm text-gray-500">This will permanently delete the student and all their enrollment records. This action cannot be undone.</p>
                    </div>
                </div>
